...
class TestVersion(UnitTestCase):
    ...
    def test_should_get_current_version_in_current_version_dot_txt(self):
        original_current_version = example_migrations.conf.current_version
        del example_migrations.conf.current_version
        example_migrations.conf.folder = "%s/example_migrations" % example_migrations.conf.abs_path
        self.assertEqual("0.0.1", self.version.get_current())
        example_migrations.conf.current_version = original_current_version
    ...
    def test_should_set_the_current_version_in_dot_txt(self):
        original_set_current_version = example_migrations.conf.set_current_version
        original_current_version = example_migrations.conf.current_version
        del example_migrations.conf.current_version
        del example_migrations.conf.set_current_version
        ...
        example_migrations.conf.current_version = original_current_version
        example_migrations.conf.current_version = original_set_current_version
